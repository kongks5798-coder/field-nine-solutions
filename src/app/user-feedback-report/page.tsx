...
export default function UserFeedbackReport() {
  ...
  const stats = useMemo(() => {
    ...
    const avg = Math.round(logs.reduce((sum, l) => sum + Number(l.score ?? 0), 0) / count * 10) / 10;
    const low = logs.filter(l => Number(l.score ?? 0) <= 2).length;
    const high = logs.filter(l => Number(l.score ?? 0) >= 4).length;
            ...
            <tbody>
              {logs.slice().reverse().map((log, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                  ...
                  <td style={{ color: Number(log.score ?? 0) >= 4 ? '#4caf50' : Number(log.score ?? 0) >= 3 ? '#ff9800' : '#f44336', fontWeight: 700 }}>{String(log.score ?? '')}</td>
                  <td style={{ whiteSpace: 'pre-line' }}>{String(log.feedback ?? '')}</td>
                </tr>
              ))}
            </tbody>
          </table>
