...
function WorkspaceIDE() {
  ...
  useEffect(() => {
    ...
    // Auto-enable Sandpack only when explicit npm library CDN imports are found
    // (isComplexClass check removed — large vanilla JS classes should NOT use Sandpack)
    const complexKeywords = /from\s+['"]three['"]/i; // only for actual npm three.js
    if (complexKeywords.test(allContent)) setSandpackMode(true);
  }, [files]);
  ...
  const autoDetectFramework = (updatedFiles: Record<string, { content: string }>) => {
    ...
    // Only auto-enable Sandpack for explicit React imports — not for vanilla canvas/game code
    // (large vanilla JS apps work better in iframe than Sandpack)
  };
