...
function WorkspaceIDE() {
  ...
  useEffect(() => {
    ...
    if (needsSandpack) { setSandpackMode(true); return; }

    // Auto-enable Sandpack for complex apps (game engines, data viz, animation libs)
    const jsFile = Object.values(files).find(f => f.language === "javascript");
    const jsContent = jsFile?.content ?? "";
    const complexKeywords = /canvas\s*\.|three\.js|phaser|p5\.js|chart\.js|d3\.js|anime\.js/i;
    const isComplexClass = /class\s+\w+\s*\{[\s\S]{500,}/.test(jsContent);
    if (complexKeywords.test(allContent) || isComplexClass) setSandpackMode(true);
  }, [files]);
  ...
  useEffect(() => {
    ...
    const AUTO_FIX_DELAY = 1; // 카운트다운 초
  ...
  const autoDetectFramework = (updatedFiles: Record<string, { content: string }>) => {
    ...
    // Also auto-enable for complex game/viz patterns generated by AI
    const complexKeywords = /canvas\s*\.|three\.js|phaser|p5\.js|chart\.js|d3\.js|anime\.js/i;
    const isComplexClass = /class\s+\w+\s*\{[\s\S]{500,}/.test(allContent);
    if (complexKeywords.test(allContent) || isComplexClass) setSandpackMode(true);
  };
